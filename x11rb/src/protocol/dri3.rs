...
pub fn buffer_from_pixmap<Conn>(
    conn: &Conn,
    pixmap: xproto::Pixmap,
) -> Result<CookieWithFds<'_, Conn, BufferFromPixmapReply>, ConnectionError>
where
    Conn: RequestConnection + ?Sized,
{
    let request0 = BufferFromPixmapRequest { pixmap };
    let (bytes, fds) = request0.serialize(major_opcode(conn)?);
    let slices = [IoSlice::new(&bytes[0])];
    assert_eq!(slices.len(), bytes.len());
    conn.send_request_with_reply_with_fds(&slices, fds)
...
where
    Conn: RequestConnection + ?Sized,
{
    let request0 = FDFromFenceRequest { drawable, fence };
    let (bytes, fds) = request0.serialize(major_opcode(conn)?);
    let slices = [IoSlice::new(&bytes[0])];
    assert_eq!(slices.len(), bytes.len());
    conn.send_request_with_reply_with_fds(&slices, fds)
...
pub fn get_supported_modifiers<Conn>(
    conn: &Conn,
    window: u32,
    depth: u8,
    bpp: u8,
) -> Result<Cookie<'_, Conn, GetSupportedModifiersReply>, ConnectionError>
where
    Conn: RequestConnection + ?Sized,
{
...
pub fn pixmap_from_buffers<Conn>(
    conn: &Conn,
    pixmap: xproto::Pixmap,
...
pub fn buffers_from_pixmap<Conn>(
    conn: &Conn,
    pixmap: xproto::Pixmap,
) -> Result<CookieWithFds<'_, Conn, BuffersFromPixmapReply>, ConnectionError>
where
    Conn: RequestConnection + ?Sized,
{
    let request0 = BuffersFromPixmapRequest { pixmap };
    let (bytes, fds) = request0.serialize(major_opcode(conn)?);
    let slices = [IoSlice::new(&bytes[0])];
    assert_eq!(slices.len(), bytes.len());
    conn.send_request_with_reply_with_fds(&slices, fds)
...
pub fn set_drm_device_in_use<Conn>(
    conn: &Conn,
    window: xproto::Window,
    drm_major: u32,
    drm_minor: u32,
...
where
    Conn: RequestConnection + ?Sized,
{
    let request0 = FreeSyncobjRequest { syncobj };
    let (bytes, fds) = request0.serialize(major_opcode(conn)?);
    let slices = [IoSlice::new(&bytes[0])];
    assert_eq!(slices.len(), bytes.len());
    conn.send_request_without_reply(&slices, fds)
...
pub trait ConnectionExt: RequestConnection {
    ...
    {
        pixmap_from_buffer(
            self, pixmap, drawable, size, width, height, stride, depth, bpp, pixmap_fd,
        )
    }
    ...
    fn dri3_pixmap_from_buffers(
        ...
        pixmap_from_buffers(
            self, pixmap, window, width, height, stride0, offset0, stride1, offset1, stride2,
            offset2, stride3, offset3, depth, bpp, modifier, buffers,
        )
    }
